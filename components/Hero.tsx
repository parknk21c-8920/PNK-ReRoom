    ...
    <section id="top" className="relative w-full overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-6xl flex-col items-center px-6 pb-28 pt-24 text-center md:pb-32 md:pt-32">
        <Reveal className="flex flex-col items-center">
          {/* 배지 */}
          <p className="mb-7 flex items-center gap-2.5 rounded-full border border-line bg-paper-raised/80 px-4 py-1.5 text-[13px] font-semibold tracking-wide text-ink-soft backdrop-blur-sm">
          ...
          <dl className="mt-14 flex items-center divide-x divide-line">
            {STATS.map((stat) => (
              <div key={stat.label} className="px-7 sm:px-10">
                ...
                <dd className="font-display text-2xl font-bold tracking-tight text-ink md:text-3xl">
                  {stat.value}
                </dd>
                <dd className="mt-1.5 text-[12px] font-medium uppercase tracking-[0.12em] text-ink-faint">
                  {stat.label}
                </dd>
              </div>
            ))}
